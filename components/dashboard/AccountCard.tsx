                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="bg-popover border-border text-popover-foreground">
                                <button
                                    ...
                                    className="flex w-full items-center px-2 py-1.5 text-sm outline-none hover:bg-muted text-destructive cursor-pointer transition-colors rounded-sm"
                                >
                ...
                <AlertDialogContent className="bg-card border-border sm:max-w-[425px]">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-muted-foreground">
                            Are you sure you want to delete <strong>"{account.name}"</strong>? This will permanently erase the account and all of its associated trade history. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="mt-4">
                        <AlertDialogCancel className="bg-muted text-foreground hover:bg-muted/80 border-0">Cancel</AlertDialogCancel>
                        <form action={async () => {
                            await deleteAccount(account.id)
                            setIsDeleteDialogOpen(false)
                        }}>
                            <AlertDialogAction type="submit" className="bg-destructive text-destructive-foreground hover:bg-destructive/90 w-full sm:w-auto">
                                Delete Account
                            </AlertDialogAction>
                        </form>
                    </AlertDialogFooter>
